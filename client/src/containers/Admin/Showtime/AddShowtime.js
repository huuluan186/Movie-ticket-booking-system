...
import { apiCreateShowtime } from '../../../services/showtime';
...
const AddShowtime = () => {
    ...
    const handleSubmit = async () => {
        ...
        const response = await apiCreateShowtime(submitPayload, selectedMovie.id);
        if (response?.err === 0) {
            toast.success(response.msg);
            //Fetch lại danh sách sau khi tạo thành công
            dispatch(actions.getShowtime());
            setTimeout(() => {
    ...
    return (
        <div className='space-y-4'>
            <ContainerBox>
                <TitleHeader title="Thêm suất chiếu"/>
            </ContainerBox>
            <ContainerBox>
